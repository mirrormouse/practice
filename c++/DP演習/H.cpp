...
long solve(long i,vector<vector<long>>& A,long N){
...
if(AL==0){
    ...
    long ans=0;
    for(long j=0;j<AL;j++){
        if(memo.at(A.at(i).at(j))!=-1){
            if(ans<memo.at(A.at(i).at(j))){
                ans=memo.at(A.at(i).at(j))
            }
        }else{
            long s=solve(A.at(i).at(j),A,N)
            memo.at(A.at(i).at(j))=s;
            if(ans<memo.at(A.at(i).at(j))){
                ans=memo.at(A.at(i).at(j))
            }
        }
...
vector<vector<long>> c(H,vector<long>(W,0));
c.at(0).at(0)=1;
for(long i=0;i<H;i++){
    for(long j=0;j<W;j++){
        cin>>a.at(i).at(j);
...
for(long i=0;i<H;i++){
    for(long j=0;j<W;j++){
        if(i-1>0&&j-1>0&&a.at(i-1).at(j)=='.'&&a.at(i).at(j-1)=='.'){
            a.at(i).at(j)=a.at(i-1).at(j)+a.at(i).at(j-1);
        }else if(i-1>0&&a.at(i-1).at(j)=='.'){
            a.at(i).at(j)=a.at(i-1).at(j);
        }else if(j-1>0&&a.at(i).at(j-1)=='.'){
