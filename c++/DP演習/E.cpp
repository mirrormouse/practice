...
vector<vector<long>> dp(N+1,vector<long>(sumv+1,0));//dp.at(i).at(j)　i個目までの品物で価値j以上の一番軽い場合

for(long i=1;i<N+1;i++){
    for(long j=1;j<sumv+1;j++){
        if(j>=v.at(i-1)){
            dp.at(i).at(j)=Min2(dp.at(i-1).at(j-v.at(i-1))+w.at(i-1),dp.at(i-1).at(j));
        }else{
