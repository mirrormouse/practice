...
for(long i=1;i<N+1;i++){
    for(long j=1;j<K+1;j++){
        if(j-1-a.at(i-1)>=0){
            dp.at(i).at(j)=(dp.at(i).at(j-1)+dp.at(i-1).at(j)-dp.at(i-1).at(j-a.at(i-1)-1))%MOD;
        }else{
            dp.at(i).at(j)=(dp.at(i).at(j-1)+dp.at(i-1).at(j))%MOD;
        }
...
for(long i=0;i<N+1;i++){
    for(long j=0;j<K+1;j++){
		cout<<i<<","<<j<<":"<<dp.at(i).at(j)<<endl;
    }
}

cout<<(dp.at(N).at(K)+MOD)%MOD<<endl;
