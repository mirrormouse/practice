...
vector<bool> seen1;  // 探索したか記録

P solve(vector<vector<long>>& G,vector<P>& Ans,long now){
    long w_ans=1;
    long b_ans=1;
    P s_pair;
    for(auto e : G[now])
    {
        if(!seen1[e]){
            //s_pair=solve(G,Ans,seen1,e);
            s_pair=solve(G,Ans,e);
            seen1[e]=true;
        }
        w_ans=w_ans*s_pair.first*s_pair.second;
        b_ans=b_ans*s_pair.first;
    }

    P answer=make_pair(w_ans,b_ans);
    return answer;
...
seen1.assign(N+1,false);//N個のfalse
seen1[1]=true;
//P An=solve(G,Ans,seen1,1);
  P An=solve(G,Ans,1);
