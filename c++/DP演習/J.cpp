...
//dp.at(i).at(j).at(k) 1個以上i皿2個以上j皿3個以上k皿
dp.at(0).at(0).at(0)=1;
    ...
    for(long double j=0;j<N;j--){
        for(long double k=0;k<N;k--){
            if(i==0&&j==0&&k==0){
                ...
                if(k>0){
                    dp.at(i).at(j).at(k)=(((i-j)/N)*dp.at(i-1).at(j).at(k)+((j-k)/N)*dp.at(i).at(j-1).at(k)+(k/N)*dp.at(i).at(j).at(k-1)+1)*(N/i);     
                }else if(j>0){
                    dp.at(i).at(j).at(k)=(((i-j)/N)*dp.at(i-1).at(j).at(k)+((j-k)/N)*dp.at(i).at(j-1).at(k)+1)*(N/i);     
                }else{
                    dp.at(i).at(j).at(k)=(((i-j)/N)*dp.at(i-1).at(j).at(k)+1)*(N/i);     
                }                
            }
        }
    }
...
long double answer=dp.at(a1).at(a2).at(a3);
