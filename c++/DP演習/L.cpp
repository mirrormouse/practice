...
cin>>N;
vector<long> a(N);
vector<vector<long>> b(N,vector<long>(N));
for(long i=0;i<N;i++){
cin>>a.at(i);
b.at(i).at(i)=a.at(i);
}
for(long i=1;i<N;i++){
    ...
    for(long j=0;j+i<N;j++){
            long foA=b.at(j).at(j+i-1)+a.at(j+i);
            long baA=b.at(j+1).at(j+i)+a.at(j);
            long A=Max2(foA,baA);
            long foB=b.at(j).at(j+i-1)-a.at(j+i);
            long baB=b.at(j+1).at(j+i)-a.at(j);
            long B=Min2(foB,baB);            
            if(num%2==1){//X-Yを大きくしたい
                b.at(i).at(j+i)=A;
            }else{
                b.at(i).at(j+i)=B;
            }
